pca_plot <- function(
  ...
  if(ncol(pop_order) >= 5){
    egv_order$color <- as.factor(pop_order[,5])
    col_df <- unique(pop_order[,4:5])
    col_df <- col_df[order(col_df[,1]),]
    print(col_df)
  }
  egv_order$group2 <- pop_order[,7]
  egv_order$group2 <- as.factor(egv_order$group2)
  if(ncol(pop_order) >= 5){
    egv_order$sha <- as.factor(pop_order[,6])
    sha_df <- unique(pop_order[,6:7])
    sha_df <- sha_df[order(sha_df[,1]),]
    print(sha_df)
  }
  ...
  if(shape == T){
    p12 <- p12 + geom_point(aes(shape = sha, fill = group), size = size, alpha = 1)
    p13 <- p13 + geom_point(aes(shape = sha, fill = group), size = size, alpha = 1)
    p23 <- p23 + geom_point(aes(shape = sha, fill = group), size = size, alpha = 1)
  }else{
  ...
  ### PC1 and PC2 plot
  if(!is.null(shapes)){
    p12 <- p12 + scale_shape_manual(labels = sha_df$group2,values = sha_df$pch)
  }
  ...
  ### PC1 and PC3 plot
  if(!is.null(shapes)){
    p13 <- p13 + scale_shape_manual(labels = sha_df$group2,values = sha_df$pch)
  }
  ...
  ### PC2 and PC3 plot
  if(!is.null(shapes)){
    p23 <- p23 + scale_shape_manual(labels = sha_df$group2,values = sha_df$pch)
  }
